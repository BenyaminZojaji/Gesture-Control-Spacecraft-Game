...
class Game(arcade.Window):
    def __init__(self):
        self.w = 800
        self.h = 600
        super().__init__(self.w, self.h, title="Gesture Control Spacecraft Game by Benyamin Zojaji")
    ...
    def on_update(self, delta_time):
        ...
        if self.end_time - self.start_time > self.next_enemy_time:
            ...
            self.speedOverTime += 0.1
            self.enemy_list.append(Enemy(self.w, self.h, self.speedOverTime))
            self.start_time = time.time()
        self.me.rotate()
        self.me.move()
        for i in range(len(self.me.bullet_list)):
